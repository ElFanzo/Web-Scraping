...
def get_stock_info(stock, file):
    ...
    last_price = g.doc.select('//*[@id="quote-header-info"]/div[3]/div[1]/div/span[1]').text()
    change, change_perc = g.doc.select('//*[@id="quote-header-info"]/div[3]/div[1]/div/span[2]').text().split()
    curr = g.doc.select('//*[@id="quote-header-info"]/div[2]/div[1]/div[2]/span').text()[-3:]
    ...
    mark_cap = g.doc.select('//*[@id="quote-summary"]/div[2]/table/tbody/tr[1]/td[2]/span').text()

    file.write(stock + ',')
    file.write(','.join(i for i in [last_price, change, change_perc, curr, mark_time, vol, avg_vol, day_r, week_r, mark_cap, '\n']))
...
def get_options_info(stock):
    ...
    for i, j in zip([1, 2], ['Calls', 'Puts']):
        file.write('%s\n' % j)
        rows = g.doc.select('//*[@id="Col1-1-OptionContracts-Proxy"]/section/section[%d]/div[2]/div/table/tbody/tr' % i)
        for row in rows:
