...
def getData(stock, _date):
    date_string = ''
    try:
        date_string = '&date=%d' % ((datetime.strptime(_date, '%d%m%Y').date() - date(2018, 8, 3)).total_seconds()
                                    + 1533254400)
        _date = '_' + str(_date)
    except ValueError:
        _date = ''
    req = get('https://finance.yahoo.com/quote/{0}/options?p={0}{1}'.format(stock, date_string))
